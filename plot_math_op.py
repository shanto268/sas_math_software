"""
TO DO:
    -> add derivative on screen
    -> add integral on screen 
    -> add values at derivative and inegral
"""

from dearpygui.dearpygui import *
from math import sin, cos, tan, factorial, exp, log, sqrt, log1p, log2, log10, acos, asin, atan, atan2, acosh, asinh
from math import atanh, e, pi, fmod, expm1, pow, hypot, degrees, radians, erf, erfc, gamma, lgamma, fabs, cosh, sinh, tanh
import numpy as np
...
def plot_callback(sender, data):
    ...
    data1 = data = run_code(get_value("Y1"), ranges[0], ranges[1],
                            get_value("Points"))

    add_line_series("Plot1",
                    "Y1",
                    data1,
                    weight=2,
                    color=get_value("Color"),
                    fill=get_value("Fill"))
    add_scatter_series("Plot1",
                       "Y1 Scatter",
                       data1,
                       weight=2,
                       outline=get_value("Color"),
                       fill=get_value("Fill"))
    add_line_series("Plot2",
                    "Y1",
                    data1,
                    weight=2,
                    color=get_value("Color"),
                    fill=get_value("Fill"))
    add_scatter_series("Plot2",
                       "Y1 Scatter",
                       data1,
                       weight=2,
                       outline=get_value("Color"),
...
def integral_calc_callback(sender, data):
    ranges = get_value("Range")
    data1 = data = run_code(get_value("Y1"), ranges[0], ranges[1],
                            get_value("Points"))
    xvals, yvals = [list(i) for i in zip(*data1)]
    integral = abs(integrate.simps(xvals, yvals))
    integral_statement = "The integral is approximately: "
    add_data("integral_val", integral_statement + str(integral))


set_main_window_title("SAS Math Software")
...
add_spacing(count=4)
add_button("Calculate Integral", callback=integral_calc_callback)
add_label_text("", "", data_source="integral_val")
add_spacing(count=20)
add_color_picker3("Color",
                  default_value=[255, 0, 0, 255],
                  callback=plot_callback)
add_color_picker3("Fill",
                  default_value=[255, 0, 0, 100],
                  callback=plot_callback)
end()
add_same_line()
